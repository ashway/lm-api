...
router.get('/car/list/:model', async function (ctx, next) {
    ...
    if(rows && _.isArray(rows) && rows.length==0) {
        rows = await db.all("SELECT carcatalog.* from carcatalog INNER JOIN carmodel ON carmodel.alias=carcatalog.model WHERE carmodel.class=$class ORDER BY carcatalog.active DESC, carcatalog.alias ASC", { $class: ctx.params.model });
    }
